...
from app.models import Medicines,ProductItems,ReviewForm,MyOrders,CustomUser,Ayurveda,BlogPost, Symptoms_medicine,Video,Prescription,Skincare
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('app:custom_login')
            
        else:
            print(form.errors)
            messages.success(request, 'Account created successfully. Please log in.')
            return redirect('app:custom_login')  # Redirect to login page
    else:
        form = CreateUserForm()
...
def HandleLogout(request):
    if request.method == 'POST':
        logout(request)
        return redirect('app:custom_login')  # Redirect to your login page URL pattern
...
def myorders(request):
    ...
    # Fetching the data from MyOrders table appd on email id
    ...
    if request.method == "POST":
        ...
        # Retrieving the price appd on the selected item
        price = None
...
def medicine_detail(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your review has been submitted successfully!') 
            return redirect('app:medicine_detail', product_id=product.id)
    else:
        form = ReviewForm()
...
def ayurveda_detail(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your review has been submitted successfully!') 
            return redirect('app:ayurveda_detail', product_id=product.id)
    else:
        form = ReviewForm()
...
def skincare_detail(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your review has been submitted successfully!') 
            return redirect('app:skincare_detail', product_id=product.id)
    else:
        form = ReviewForm()
...
def process_prescription(request):
    if request.method == 'POST':
        ...
        return redirect('app:confirm_medicines', matching_products=matching_product_ids)
...
def filter_medicines(confirmed_medicines):
    ...
    # Retrieve all medicine objects from the dataapp
    ...
    for confirmed_med in confirmed_medicines:
        ...
        for medicine in medicine_objects:
            ...
            # No partial match, perform token-appd comparison
            else:
